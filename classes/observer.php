...
class mod_finalgrade_observer {
    ...
    public static function final_grades_regraded(\core\event\user_graded $event) {
        ...
        if ($modules = $DB->get_records_sql($sql, array($event->context->get_course_context()->id))) {

            $grade_grades = grade_grade::fetch_users_grades($event->objectid, array($event->relateduserid), true);
